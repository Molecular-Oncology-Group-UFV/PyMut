...
from .core import MutationMetadata, PyMutation
from .utils.format import format_chr, format_rs, normalize_variant_classification
...
def _consolidate_variants(maf_df, samples):
    ...
    logger = logging.getLogger(__name__)  # Asegura que el logger esté definido
    start_time = time.time()
    logger.info("Consolidating variants using optimized pure NumPy arrays...")

    group_cols = ['CHROM', 'POS', 'REF', 'ALT']
    # Extraemos solo las anotaciones (excluyendo samples y group_cols para evitar solapamientos)
    annotation_cols = [c for c in maf_df.columns if c not in samples and c not in group_cols]

    # 1. Separar filas únicas de las duplicadas
    # Esto salva el ~95% del tiempo de cómputo
    dupe_mask = maf_df.duplicated(subset=group_cols, keep=False)

    if not dupe_mask.any():
        # Si no hay variantes duplicadas, pasamos directo al final
        final_df = maf_df.copy()
    else:
        df_unique = maf_df[~dupe_mask]
        df_dupes = maf_df[dupe_mask]
        ...
        sample_matrix = df_dupes[samples].values
        ref_genotypes = (df_dupes['REF'] + '|' + df_dupes['REF']).values[:, None]

        # Reemplazo de genotipos
        sample_matrix_nan = np.where(sample_matrix == ref_genotypes, np.nan, sample_matrix)
        df_samples_only = pd.DataFrame(sample_matrix_nan, columns=samples, index=df_dupes.index)
        df_dupes_samples = pd.concat([df_dupes[group_cols], df_samples_only], axis=1)

        # Agrupamos SOLO el subconjunto pequeño que está duplicado
        consolidated_samples = df_dupes_samples.groupby(group_cols, dropna=False, as_index=False).first()

        # 4. Unimos anotaciones y muestras de los duplicados
        final_dupes = pd.merge(ann_dupes, consolidated_samples, on=group_cols)

        # 5. Juntamos las filas únicas (que no tocamos) con las consolidadas
        final_df = pd.concat([df_unique, final_dupes], ignore_index=True)

    # 6. Rellenamos los NaN (Tu fix estructural final, aplicado a todo)
    final_sample_matrix = final_df[samples].values
    final_ref_genotypes = (final_df['REF'] + '|' + final_df['REF']).values[:, None]

    filled_matrix = np.where(pd.isna(final_sample_matrix), final_ref_genotypes, final_sample_matrix)

    # Construimos bloques limpios y pegamos de golpe, reseteando índices para evitar desajustes
    df_annotations_clean = final_df.drop(columns=samples).reset_index(drop=True)
    df_samples_clean = pd.DataFrame(filled_matrix, columns=samples).reset_index(drop=True)

    result_df = pd.concat([df_annotations_clean, df_samples_clean], axis=1)

    logger.info("Variant consolidation completed in %.2f seconds", time.time() - start_time)
...
def read_maf(path: str | Path, assembly: str, cache_dir: Optional[str | Path] = None, consolidate_variants: bool = True) -> PyMutation:
    ...
    # Separate comments from body and reconstruct split lines
    comments: list[str] = []
    buf = io.StringIO()
    try:
        with _open_text_maybe_gzip(path) as fh:
            ...
            for line in fh:
                ...
                # Use the header to determine the expected number of fields
                if expected_fields is None:
                    expected_fields = line.count("\t") + 1
                    buf.write(line)
                    continue

                # If there is a pending incomplete line, join it with the current
            ...
            if pending is not None:
                logger.warning(
                    "Trailing incomplete line in MAF (%d/%d fields), writing as-is.",
                    pending.count("\t") + 1,
                    expected_fields,
                )
                buf.write(pending)
