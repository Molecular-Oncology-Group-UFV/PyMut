...
# Clean up empty container annotations so they appear as empty (NaN) instead of []
def _clean_empty_annotations(val):
    if val is None:
        return None
    # Convert empty containers to NaN
    try:
        if isinstance(val, (list, tuple, set, dict)) and len(val) == 0:
            return np.nan
    except Exception:
        pass
    # Handle empty or placeholder string representations
...
class ActionableMutationMixin:
    def actionable_mutations_oncokb(self, token: str, batch_size: int = 5000, timeout: int = 30,
        ...
        if annotations_dict:
            ...
            oncokb_columns = [c for c in self.data.columns if c.startswith("oncokb_")]
            for c in oncokb_columns:
                # For boolean-like flags, convert False/None to NaN so stats don't count them
                if self.data[c].dtype == bool or c in ["oncokb_hotspot", "oncokb_vus"]:
                    self.data[c] = self.data[c].map(lambda v: True if v is True else (np.nan if (v is False or v is None) else v))
                    continue
                # Clean empty containers and empty strings
                self.data[c] = self.data[c].apply(_clean_empty_annotations)
