...
from pyMut.analysis.mutational_signature import TRINUCLEOTIDE_CONTEXTS


def _fake_extract_signatures(contexts_df, *args, **kwargs):
    """Deterministic stand-in for extractSignatures() (R NMF via rpy2).

    These are PLOTTING smoke tests: what must not break is the figure code,
    not the NMF solver. A real NMF run is also impossible here because the
    tiny 3-mutation fixture matrix has 93 all-zero context rows, and R's
    NMF::nmf rejects matrices with zero rows.
    """
    k = args[0] if args else kwargs.get("n", 2)
    n_samples = contexts_df.shape[1] if hasattr(contexts_df, "shape") else 2
    rng = np.random.default_rng(0)
    w = rng.random((96, k)) + 0.1
    w /= w.sum(axis=0, keepdims=True)
    h = rng.random((k, n_samples)) + 0.1
    h /= h.sum(axis=0, keepdims=True)
    w_df = pd.DataFrame(w, index=TRINUCLEOTIDE_CONTEXTS,
                        columns=[f"Signature {i + 1}" for i in range(k)])
    h_df = pd.DataFrame(h, index=w_df.columns, columns=list(contexts_df.columns))
    return {
        "signatures": w_df,
        "contributions": h_df,
        "nmfObj": None,
        "contributions_abs": h_df * 100,
    }


def _start_nmf_patch():
    """Patch extractSignatures everywhere the plotting code may look it up.

    The plotting methods may resolve the function either at module level
    (pyMut.analysis.mutational_signature.extractSignatures) or as a
    PyMutation method, so patch both. Unknown module layouts are skipped.
    """
    patchers = []
    try:
        import pyMut.analysis.mutational_signature as ms_mod
        patchers.append(mock.patch.object(ms_mod, "extractSignatures", _fake_extract_signatures))
    except Exception:
        pass
    try:
        import src.pyMut.analysis.mutational_signature as ms_mod_src
        patchers.append(mock.patch.object(ms_mod_src, "extractSignatures", _fake_extract_signatures))
    except Exception:
        pass
    if hasattr(PyMutation, "extractSignatures"):
        patchers.append(mock.patch.object(
            PyMutation, "extractSignatures",
            lambda self, ctx, *a, **k: _fake_extract_signatures(ctx, *a, **k),
        ))
    for p in patchers:
        p.start()
    return patchers
...
class TestMutationalSignature(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Smoke-test the plotting code, not the NMF solver: replace the real
        # extractSignatures (R NMF via rpy2) with a deterministic fake. The
        # real implementation rejects the tiny fixture matrix because it has
        # all-zero context rows, which is a data property, not a plotting bug.
        self._nmf_patchers = _start_nmf_patch()

    def tearDown(self):
        for p in getattr(self, "_nmf_patchers", []):
            p.stop()
