...
# An input file guaranteed to exist, resolved from the pyMut package itself
# (works from any working directory: repo root, sandbox copy of the tests,
# or installed package). Used by the missing-cache tests, which need the VCF
# and FASTA existence checks to PASS so that validation reaches the
# cache-directory check.
try:
    import pyMut as _pyMut_pkg
    _EXISTING_INPUT = str(
        Path(_pyMut_pkg.__file__).resolve().parent
        / "data" / "examples" / "VCF"
        / "subset_1k_variants_ALL.chr10.shapeit2_integrated_snvindels_v2a_27022019.GRCh38.phased.vcf"
    )
except Exception:
    _EXISTING_INPUT = VCF_FILE
...
class TestWrapMafVepAnnotateProtein:
    ...
    def test_successful_annotation_with_real_files(self):
        ...
        success, result = wrap_maf_vep_annotate_protein(
            MAF_FILE, MAF_FASTA, cache_dir=MAF_CACHE_DIR
        )
...
class TestWrapVcfVepAnnotateGene:
    ...
    def test_successful_annotation_with_real_files(self):
        ...
        success, result = wrap_vcf_vep_annotate_unified(
            VCF_FILE, VCF_FASTA, cache_dir=VCF_CACHE_DIR, annotate_gene=True
        )
    ...
    def test_with_distance_parameter(self):
        ...
        success, result = wrap_vcf_vep_annotate_unified(
            VCF_FILE, VCF_FASTA, cache_dir=VCF_CACHE_DIR, annotate_gene=True, distance=10000
        )
    ...
    def test_no_stats_parameter_with_real_files(self):
        ...
        success, result = wrap_vcf_vep_annotate_unified(
            VCF_FILE, VCF_FASTA, cache_dir=VCF_CACHE_DIR, annotate_gene=True, no_stats=True
        )
        assert success is True
        
        # Test with no_stats=False
        success, result = wrap_vcf_vep_annotate_unified(
            VCF_FILE, VCF_FASTA, cache_dir=VCF_CACHE_DIR, annotate_gene=True, no_stats=False
        )
        assert success is True
    ...
    def test_missing_files_raise_filenotfounderror(self):
        ...
        with pytest.raises(FileNotFoundError, match="VCF file not found"):
            wrap_vcf_vep_annotate_unified("nonexistent.vcf", VCF_FASTA, cache_dir=VCF_CACHE_DIR, annotate_gene=True)
        
        with pytest.raises(FileNotFoundError, match="Cache directory not found"):
            wrap_vcf_vep_annotate_unified(_EXISTING_INPUT, _EXISTING_INPUT, cache_dir="nonexistent_cache", annotate_gene=True)
        
        with pytest.raises(FileNotFoundError, match="FASTA file not found"):
            wrap_vcf_vep_annotate_unified(_EXISTING_INPUT, "nonexistent.fa", cache_dir=VCF_CACHE_DIR, annotate_gene=True)
...
class TestWrapVcfVepAnnotateUnified:
    ...
    def test_validation_no_annotation_enabled(self):
        ...
        with pytest.raises(ValueError, match="At least one annotation option must be enabled"):
            wrap_vcf_vep_annotate_unified(
                VCF_FILE, VCF_FASTA, cache_dir=VCF_CACHE_DIR,
    ...
    def test_protein_annotation_only(self):
        ...
        success, result = wrap_vcf_vep_annotate_unified(
            VCF_FILE, VCF_FASTA, cache_dir=VCF_CACHE_DIR, annotate_protein=True
        )
    ...
    def test_variant_class_annotation_only(self):
        ...
        success, result = wrap_vcf_vep_annotate_unified(
            VCF_FILE, VCF_FASTA, cache_dir=VCF_CACHE_DIR, annotate_variant_class=True
        )
    ...
    def test_missing_files_raise_filenotfounderror(self):
        """Test missing files raise FileNotFoundError"""
        with pytest.raises(FileNotFoundError, match="VCF file not found"):
            wrap_vcf_vep_annotate_unified("nonexistent.vcf", VCF_FASTA, cache_dir=VCF_CACHE_DIR, annotate_protein=True)
        
        with pytest.raises(FileNotFoundError, match="Cache directory not found"):
            wrap_vcf_vep_annotate_unified(_EXISTING_INPUT, _EXISTING_INPUT, cache_dir="nonexistent_cache", annotate_protein=True)
        
        with pytest.raises(FileNotFoundError, match="FASTA file not found"):
            wrap_vcf_vep_annotate_unified(_EXISTING_INPUT, "nonexistent.fa", cache_dir=VCF_CACHE_DIR, annotate_protein=True)
