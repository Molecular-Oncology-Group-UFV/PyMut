...
class TestToMAFEndPosition:
    ...
    def test_to_maf_end_position_for_other_variants(self, dummy_meta, tmp_path):
        # Insertion-like: allele2 longer than REF
        df = pd.DataFrame({
            "CHROM": ["chr1"],
            "POS": [300],
            "REF": ["A"],
        ...
        pm = DummyPyMutation(df, ["S1"], dummy_meta)
        out = tmp_path / "maf_other.tsv"
        pm.to_maf(out)
        df_out = pd.read_csv(out, sep="\t")
        assert int(df_out.loc[0, "End_Position"]) == int(df_out.loc[0, "Start_Position"])  # other
