...
def test_extractSignatures_invalid_inputs(contexts_df_small):
    # rpy2 is imported inside extractSignatures() BEFORE input validation,
    # so reaching the ValueError checks requires the rpy2 runtime.
...
def test_extractSignatures_normalization_and_pseudocount(contexts_df_small):
    # estimateSignatures() still uses scikit-learn, but extractSignatures()
    # runs R's NMF::nmf via rpy2 -> needs the same runtime as the real code.
    pytest.importorskip("rpy2")
...
def test_extractSignatures_reproducible_results(contexts_df_small):
    ...
    # two runs on the same matrix must return identical signatures and
    # contributions. Requires rpy2 + a working R with the 'NMF' package
    # (same dependency as the maftools-parity implementation).
    pytest.importorskip("rpy2")
...
def test_extractSignatures_nmf_failure_raises(contexts_df_small):
    # extractSignatures() calls R's NMF::nmf via rpy2 and must surface any
    # NMF failure as ValueError("NMF decomposition failed ..."). A genuine,
    # deterministic NMF failure is a context row of all zeros, which
    # NMF::nmf rejects ("Input matrix x contains at least one null or
    # NA-filled row"). (Note: k > n_samples only triggers an R *warning*,
    # not an error, so it cannot be used here.)
    pytest.importorskip("rpy2")
    bad = contexts_df_small.copy()
    bad.iloc[0, :] = 0
    with pytest.raises(ValueError, match="NMF decomposition failed"):
        extractSignatures(bad, n=2)
