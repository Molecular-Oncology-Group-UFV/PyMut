...
class TestSummaryPlots:
    def test_summary_plot_calls_extractors_and_helper_and_closes_figure(self, monkeypatch, df_minimo):
        ...
        def fake_helper(self_obj, figsize, title, max_samples, top_genes_count, **kwargs):
            called["helper"] += 1
            ...
            # Newer API forwards include_silent / non_syn_classifications
            assert 'include_silent' in kwargs
            return plt.figure()
        ...
        monkeypatch.setattr(
            "src.pyMut.visualizations.summary_plot._create_summary_plot",
            fake_helper,
        )
    ...
    def test_variant_classification_plot_extracts_and_sets_title(self, monkeypatch, df_minimo):
        ...
        def fake_helper(self_obj, ax, set_title, **kwargs):
            created["ax"] = ax
        ...
        monkeypatch.setattr(
            "src.pyMut.visualizations.summary_plot._create_variant_classification_plot",
            fake_helper,
        )
    ...
    def test_variant_type_plot_extracts_and_sets_title(self, monkeypatch, df_minimo):
        ...
        monkeypatch.setattr(
            "src.pyMut.visualizations.summary_plot._create_variant_type_plot",
            fake_helper,
        )
    ...
    def test_snv_class_plot_calls_helper_with_columns_and_title(self, monkeypatch, df_minimo):
        ...
        def fake_helper(self_obj, ref_column, alt_column, ax, set_title, **kwargs):
            seen["ref"] = ref_column
            seen["alt"] = alt_column
            assert set_title is False
            return ax
        ...
        monkeypatch.setattr(
            "src.pyMut.visualizations.summary_plot._create_snv_class_plot",
            fake_helper,
        )
    ...
    def test_variants_per_sample_plot_normalizes_variant_column_and_title_y_when_custom(
        ...
        def fake_helper(self_obj, variant_column, sample_column, ax, set_title, max_samples, **kwargs):
        ...
        monkeypatch.setattr(
            "src.pyMut.visualizations.summary_plot._create_variants_per_sample_plot",
            fake_helper,
        )
    ...
    def test_variant_classification_summary_plot_detects_wide_format_and_calls_helper(
        self, monkeypatch, df_wide_format, caplog
    ):
        ...
        def fake_helper(self_obj, variant_column, sample_column, ax, set_title, show_labels=None, **kwargs):
            seen["variant_column"] = variant_column
            seen["sample_column"] = sample_column
        ...
        monkeypatch.setattr(
            "src.pyMut.visualizations.summary_plot._create_variant_classification_summary_plot",
            fake_helper,
        )

        caplog.clear()
        with caplog.at_level('DEBUG', logger='src.pyMut.core'):
            fig = pm.variant_classification_summary_plot()
        # The wide-format detection message is logged at DEBUG level in core.py
        assert any('Detected wide format with' in r.message for r in caplog.records)
...
class TestTopMutatedGenesPlot:
    ...
    def test_top_mutated_genes_plot_normalizes_columns_and_sets_expected_title(
        ...
        def fake_helper(self_obj, mode, variant_column, gene_column, sample_column, count, ax, set_title, **kwargs):
            seen.update(
                mode=mode,
        ...
        monkeypatch.setattr(
            "src.pyMut.visualizations.summary_plot._create_top_mutated_genes_plot",
            fake_helper,
        )
